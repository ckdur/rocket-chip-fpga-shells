...
class XilinxSakuraXMIGPads(depth : BigInt) extends SakuraXMIGIODDR(depth) {
  def this(c : XilinxSakuraXMIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxSakuraXMIGIO(depth : BigInt) extends SakuraXMIGIODDR(depth) with SakuraXMIGIOClocksReset

class XilinxSakuraXMIGIsland(c : XilinxSakuraXMIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  require((depth<=0x8000000L),"sakuraxmig supports up to 128MB depth configuration")
  ...
  class Impl extends LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxSakuraXMIGIO(depth)
    })
    ...
    val blackbox = Module(new sakuraxmig(depth))
...
class XilinxSakuraXMIG(c : XilinxSakuraXMIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxSakuraXMIGIsland(c, crossing))
