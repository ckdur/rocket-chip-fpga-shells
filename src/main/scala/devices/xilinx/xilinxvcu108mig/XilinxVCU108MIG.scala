...
import sifive.fpgashells.ip.xilinx.vcu108mig.{VCU108MIGIOClocksReset, VCU108MIGIODDR, vcu108mig}

case class XilinxVCU108MIGParams(
                                  address : Seq[AddressSet]
                                )

class XilinxVCU108MIGPads(depth : BigInt) extends VCU108MIGIODDR(depth) {
  def this(c : XilinxVCU108MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxVCU108MIGIO(depth : BigInt) extends VCU108MIGIODDR(depth) with VCU108MIGIOClocksReset

class XilinxVCU108MIGIsland(c : XilinxVCU108MIGParams)(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  val crossing = AsynchronousCrossing(8)
  require((depth<=0x80000000L),"vcu108mig supports upto 2GB depth configuraton")
  ...
  class Impl extends LazyModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxVCU108MIGIO(depth)
    })

    //MIG black box instantiation
    val blackbox = Module(new vcu108mig(depth))
...
class XilinxVCU108MIG(c : XilinxVCU108MIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxVCU108MIGIsland(c))
