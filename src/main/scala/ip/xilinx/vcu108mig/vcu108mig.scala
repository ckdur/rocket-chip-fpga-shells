package sifive.fpgashells.ip.xilinx.vcu108mig
...
// IP VLNV: xilinx.com:customize_ip:vcu108mig:1.0
// Black Box

class VCU108MIGIODDR(depth : BigInt) extends Bundle {
  require((depth<=0x80000000L),"VCU108MIGIODDR supports upto 2GB depth configuration")
...
//reused directly in io bundle for sifive.blocks.devices.xilinxvcu108mig
trait VCU108MIGIOClocksReset extends Bundle {
  //inputs
...
class vcu108mig(depth : BigInt)(implicit val p:Parameters) extends BlackBox
{
  require((depth<=0x80000000L),"vcu108mig supports upto 2GB depth configuraton")

  val io = IO(new VCU108MIGIODDR(depth) with VCU108MIGIOClocksReset {
  ...
  ElaborationArtefacts.add(
    "vcu108mig.vivado.tcl",
    """
      create_ip -vendor xilinx.com -library ip -version 2.2 -name ddr4 -module_name vcu108mig -dir $ipdir -force
